   ...
   const [datosEmpleado, setDatosEmpleado]=useState({
    ...
    const columns=[
        ...
        },
        {
            ...
            selector:'category',
            sortable:true,
            cell:row=><>{parseInt(row.category)===4?"Gerente":parseInt(row.category)===1?'Vendedor JR':parseInt(row.category)===2?'Vendedor Sr':parseInt(row.category)===3?'Supervisor':parseInt(row.category)===5?'Admin':''}</>
